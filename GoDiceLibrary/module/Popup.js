...
link.href = getLibraryPath()+"style/popup.css";
link.media = "all";
...
function getLibraryPath(){
	let script = Array.from(document.getElementsByTagName("script")).filter((script) => { return script.src.includes("GoDiceLibrary")});
	let fullPath = (script && script.length>0)?script[0].src:null;
	let path = fullPath?fullPath.substring(0,fullPath.lastIndexOf("GoDiceLibrary")+"GoDiceLibrary".length+1):"/";
...
export class Popup {
    ...
    init() {
        ...
        for (let i = 0; i < this.content.length; i++) {
            ...
            } else if (!this.params.htmlcontent) {
                line = `<p>${line}</p>`;
            }
...
function postHide(popup) {
    ...
    if (popup.params.hideCallback && typeof popup.params.hideCallback == "function") {
        popup.params.hideCallback(popup);
    }
    enableScroll();
}
